...
const BreathingContainer = React.memo(({ intention, onToggle }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt: Cleanup to prevent memory leaks on native thread from orphaned Animated.loop
    return () => {
      if (animation) {
        animation.stop();
      }
...
export default function App() {
  ...
  // ⚡ Bolt: Wrapped in useCallback to preserve reference and prevent child re-renders
