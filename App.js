...
// ⚡ Bolt Optimization: Wrapped in React.memo to prevent O(n) re-renders when parent state changes.
const BreathingContainer = React.memo(({ intention, onToggle }) => {
  const pulseAnim = useRef(new Animated.Value(1)).current;

  useEffect(() => {
    // ⚡ Bolt Optimization: Capture loop reference to stop animation explicitly on cleanup.
