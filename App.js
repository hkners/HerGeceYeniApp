...
// ⚡ Bolt: Wrapped in React.memo to prevent O(n) re-renders.
// Only the row whose completed status changes will re-render.
const BreathingContainer = memo(({ intention, onToggle }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt: Fixed memory leak and background thread activity by explicitly
    // capturing the animation reference and calling .stop() on unmount/dep change.
    return () => {
      if (animation) {
        animation.stop();
      }
...
export default function App() {
  ...
  // ⚡ Bolt: Memoized with useCallback (using functional state update pattern)
  // so the reference remains stable across renders, satisfying React.memo.
